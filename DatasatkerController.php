...
class DatasatkerController extends Controller
{
    ...
    public function getberita($kode_satker, Request $request)
    {
        ...
        if (!empty($request->kode_divisi_pilihan)) {
            if ($request->kode_divisi_pilihan == "all_berita") {
                $query->orderBy("tgl_input", "desc");
            } else if ($request->kode_divisi_pilihan == "berita_umum") {
                $query->where('kode_divisi', '=', 'berita_umum');
            } else {
                $query->where('kode_divisi', '=', $request->kode_divisi_pilihan);
            }
        }

        $query->orderBy("id_berita", "desc");
        //$query->orderBy("prioritas_id", "asc");
        ...
        if ($roles == 'humas_kanwil') {
            ...
            return view('beritasatker.index_kanwil', compact('getmedia', 'getdivisi', 'satker',
                'berita_satker', 'dtsatker', 'dataPrioritas'));
        } else if ($roles == 'humas_satker') {
            //return "prety";

            return view('beritasatker.index', compact('getmedia', 'getdivisi', 'satker', 'berita_satker', 'dtsatker'));
        }
    }
    ...
    public function storeberita(Request $request)
    {
        ...
        //die;
        //echo "ini kode divisi pemilik giat :".$request->kode_divisi; die;
        //die;
        try {
            $data = [
    ...
    {
        //return "pilihkonfigurasi_kanwil";
        //return "prit";
        //return $request->kode_satker_value;
        $querysatker = $query = Satker::query();
        $querysatker->select('*');
        ...
        return view('beritasatker.pilih_konfigurasi_kanwil',
            compact('dtsatker', 'sizeBerita', 'dtBerita', 'datakonfig', 'kode_satker_value', 'priorities'));
    }
    ...
    {
        //return "tes download word";
        //return Auth::guard('satker')->user()->roles;
        //return auth()->user()->roles;
        //return auth()->user()->kode_satker;
        //return $request;
        $id_konfig = $request->id_konfig;
        $id_berita = $request->id_berita;
        $kode_satker = $request->kode_satker;
        ...
        foreach ($getBerita as $index => $news) {
            ...
            $text = $section->addText('');
            if ($news->youtube != "") {
                $text = $section->addText('LINK YOUTUBES : ', $firstStyle);
        ...
        /*-------------*/
        /*beginning of link berita website*/
        $number = 0;
        foreach ($getBerita as $index => $news) {
            $number = $index + 1;
    ...
    public function whatssapgenerate_message_today_kanwil(Request $request)
    {
        ...
        /*Tanggal Dari*/
        $dari_rekap = $request->dari_rekap;
        $explode_dari = explode('-', $dari_rekap);
        ...
        /*Tanggal Sampai*/
        $sampai_rekap = $request->sampai_rekap;
        $explode_sampai = explode('-', $sampai_rekap);
        ...
        $idPrioritas = $request->idPrioritas;
        //dd($idPrioritas);
        ...
        //dd($kode_satker_pilihan);
        if ($kode_satker_pilihan != "all") {
            //dd($kode_satker_pilihan);
            $namaSatker = DB::table('satker')->where('kode_satker', $request->kode_satker_pilihan)->first()->name;
            if ($kode_divisi_pilihan != "") {
                //dd($kode_divisi_pilihan);
                if ($kode_divisi_pilihan != "all_berita") {
                    if ($idPrioritas == "all_priors") {
                        $dtBerita = DB::select(DB::raw("select * from berita a inner join tbl_prioritas b 
                              on a.prioritas_id=b.id_prioritas where 
                              tgl_input between '$dari_rekap' and '$sampai_rekap' 
                              and a.kode_satker='$kode_satker_pilihan' and kode_divisi='$kode_divisi_pilihan'
                              order by b.skala_prioritas asc"));
                        //dd($dtBerita);
                    } else if ($idPrioritas != "all_priors") {
                        $dtBerita = DB::select(DB::raw("select * from berita a inner join tbl_prioritas b 
                              on a.prioritas_id=b.id_prioritas where 
                              tgl_input between '$dari_rekap' and '$sampai_rekap' 
                              and a.kode_satker='$kode_satker_pilihan' and kode_divisi='$kode_divisi_pilihan' 
                              and prioritas_id='$idPrioritas'
                              order by b.skala_prioritas asc"));
                        //dd($dtBerita);
                    }
                    //dd($kode_divisi_pilihan);
...
//                                on a.prioritas_id=b.id_prioritas where
//                                tgl_input='$date'
//                                and a.kode_satker='$kode_satker_pilihan'
//                                and a.kode_divisi='$kode_divisi_pilihan'
//                                order by b.skala_prioritas asc"));
                } else if ($kode_divisi_pilihan == "all_berita") {
                    //echo $dari_rekap."<br>";
                    //echo $sampai_rekap."<br>";
                    //dd("pret");
                    $date = date('Y-m-d');
                    /*sinidulu*/
                    //dd($kode_divisi_pilihan);
                    if ($idPrioritas == "all_priors") {
//                        echo $dari_rekap."<br>"; /*include*/
//                        echo $sampai_rekap."<br>"; /*include*/
//                        echo $idPrioritas."<br>";/*exclude*/
//                        echo $kode_divisi_pilihan."<br>";/*exclude*/
//                        echo $kode_satker_pilihan."<br>"; /*include*/
//                        die;
                        $dtBerita = DB::select(DB::raw("select * from berita a inner join tbl_prioritas b 
                              on a.prioritas_id=b.id_prioritas where 
                              tgl_input between '$dari_rekap' and '$sampai_rekap' 
                              ...
                              order by b.skala_prioritas asc"));
                        //dd($dtBerita);
                        //dd("sampai semua: ".$idPrioritas);
                    } else if ($idPrioritas != "all_priors") {
                        $dtBerita = DB::select(DB::raw("select * from berita a inner join tbl_prioritas b 
                              on a.prioritas_id=b.id_prioritas where 
                              tgl_input between '$dari_rekap' and '$sampai_rekap' 
                              and a.kode_satker='$kode_satker_pilihan' and prioritas_id='$idPrioritas'
                              order by b.skala_prioritas asc"));

                        //dd($dtBerita);
                    }
                    //echo $kode_divisi_pilihan; die;
                    //sudah sampai sini brojo lanjutkan
                    //dd($dtBerita);

                }
            } else if ($kode_divisi_pilihan == "") {
                /*belum ada potensi masuk ke kondisi ini*/
                //dd($kode_divisi_pilihan);
                $dtBerita = DB::table('berita')
                    ->where('kode_satker', $kode_satker_pilihan)
                    ->where('tgl_input', date('Y-m-d'))
                    ->orderBy("prioritas_id", "asc")
                    ->get();
...
//                ->get();
            ...
            if (sizeof($dtBerita) > 0) {
                ...
                $text = $section->addText('');
                $text = $section->addText($dtKonfig->pengantar . ", $getCompleteDate :", $secondtStyle);
                $text = $section->addText("", $firstStyle);
                ...
                foreach ($dtBerita as $index => $news) {
                    ...
                    //untuk enter
                    //$text = $section->addText('');
                    //echo trim($news->facebook); die;
                    if ($news->website != "") {
                        //$str_length = strlen($news->website);
                        //echo ($str_length)."<br>";
                        if (str_replace(" ", "", $news->website) != "-") {
                            $text = $section->addText('');
                            $text = $section->addText('WEBSITE', $firstStyle);
                            $text = $section->addText($news->website, $secondtStyle);
                        } else if (str_replace(" ", "", $news->website) == "-") {
                            //dont add anything
                        }
                    ...
                    if ($news->twitter != "") {
                        if (str_replace(" ", "", $news->twitter) != "-") {
                            $text = $section->addText('');
                            $text = $section->addText('X', $firstStyle);
                            $text = $section->addText($news->twitter, $secondtStyle);
                        } else if (str_replace(" ", "", $news->twitter) == "-") {
                    ...
                    if ($news->facebook != "") {
                        //echo "wossy";
                        if (str_replace(" ", "", $news->facebook) != "-") {
                            $text = $section->addText('');
                            $text = $section->addText('FACEBOOK', $firstStyle);
                            $text = $section->addText($news->facebook, $secondtStyle);
                        } else if (str_replace(" ", "", $news->facebook) == "-") {
                            //dont add anything
                        }
                    }
                    ...
                    if ($news->instagram != "") {
                        if (str_replace(" ", "", $news->instagram) != "-") {
                            $text = $section->addText('');
                            $text = $section->addText('INSTAGRAM', $firstStyle);
                            $text = $section->addText($news->instagram, $secondtStyle);
                        } else if (str_replace(" ", "", $news->instagram) == "-") {
                            //dont add anything
                        }
                    }
                    ...
                    if ($news->tiktok != "") {
                        if (str_replace(" ", "", $news->tiktok) != "-") {
                            $text = $section->addText('');
                            $text = $section->addText('TIKTOK', $firstStyle);
                            $text = $section->addText($news->tiktok, $secondtStyle);
                        } else if (str_replace(" ", "", $news->tiktok) == "-") {
                            //dont add anything
                        }
                    ...
                    if ($news->youtube != "") {
                        if (str_replace(" ", "", $news->youtube) != "-") {
                            $text = $section->addText('');
                            $text = $section->addText('YOUTUBE', $firstStyle);
                            $text = $section->addText($news->youtube, $secondtStyle);
                        } else if (str_replace(" ", "", $news->youtube) == "-") {
                            //dont add anything
                        }
                ...
                $text = $section->addText($dtKonfig->nama_kakanwil, $secondtStyle);
                $text = $section->addText('');
                $text = $section->addText("PEMBATASNYA============================================================PEMBATASNYA", $secondtStyle);
                ...
                $text = $section->addText("Dari : " . $dtKonfig->dari_pengirim, $secondtStyle);
                $text = $section->addText("");
                $text = $section->addText('Tembusan Ythhhhhhh:', $firstStyle);
                $jml_tembusan_yth = json_decode($dtKonfig->jumlah_tembusan_yth);
                foreach ($jml_tembusan_yth as $key => $dt) {
                    $nomer = $key + 1;
                    $text = $section->addText("$nomer ." . $dt, $secondtStyle);
                ...
                foreach ($dtBerita as $index => $news) {
                    foreach (json_decode($news->media_lokal) as $num => $medlok) {
                        ...
                        if (str_replace(" ", "", $explode[2]) != "-") {
                            //$text = $section->addText("testing", $secondtStyle);
                            $text = $section->addText($explode[2], $secondtStyle);
                            $text = $section->addText("", $secondtStyle);
                        }
                ...
                $nmbr_nas = $nmr;
                $nmr_nas = $nmr;
                //$text = $section->addText("MEDIA NASIONALLLLLSS nmr:".$nmr_nas);
                foreach ($dtBerita as $index => $news) {
                    foreach (json_decode($news->media_nasional) as $num_nas => $mednas) {
                        $explode_nas = explode("|||", $mednas);
                        if (str_replace(" ", "", $explode_nas[2]) != "-") {
                            $text = $section->addText($nmr_nas . ". " . $explode_nas[1], $firstStyle);
                            $nmr_nas += 1;
                            $nmbr_nas += 1;
                        }
                ...
                //return redirect()->route('getberita')->with(['warning'=>'Data Berita Hari Ini Satker ' . $namaSatker . ' Belum Ada!']);
                //echo auth()->user()->kode_satker; die;
            ...
            if ($kode_divisi_pilihan != "") {
                ...
                //echo "kode divisi ada dipilih<br>";
                //echo $kode_divisi_pilihan; die;
                if ($kode_divisi_pilihan != "all_berita") {
...
//                        echo $dari_rekap . "<br>"; /*include*/
//                        echo $sampai_rekap . "<br>"; /*include*/
//                        echo "idPrioritas :" . $idPrioritas . "<br>"; /*exclude*/
//                        echo "kodeDivisi: " . $kode_divisi_pilihan . "<br>"; /*include*/
//                        echo "kodeSatker: " . $kode_satker_pilihan . "<br>"; /*exclude*/
//                        die;
                        $dtBerita = DB::select(DB::raw("select * from berita a inner join tbl_prioritas b 
                              on a.prioritas_id=b.id_prioritas where kode_divisi='$kode_divisi_pilihan' and 
                              tgl_input between '$dari_rekap' and '$sampai_rekap' 
...
//                        echo $dari_rekap . "<br>"; /*include*/
//                        echo $sampai_rekap . "<br>"; /*include*/
//                        echo "idPrioritas :" . $idPrioritas . "<br>"; /*include*/
//                        echo "kodeDivisi: " . $kode_divisi_pilihan . "<br>"; /*include*/
//                        echo "kodeSatker: " . $kode_satker_pilihan . "<br>"; /*exclude*/
//                        die;

                        $dtBerita = DB::select(DB::raw("select * from berita a inner join tbl_prioritas b 
                              on a.prioritas_id=b.id_prioritas where kode_divisi='$kode_divisi_pilihan' 
                              and prioritas_id='$idPrioritas' and 
                              tgl_input between '$dari_rekap' and '$sampai_rekap'                               
                              order by b.skala_prioritas asc"));
                        //dd($dtBerita);
                    }
                    //memilih berita berdasarkan divisi tertentu
                    //echo "sebagian berita dipilih<br>";
//                    $dtBerita = DB::table('berita')
...
//                        echo $sampai_rekap . "<br>"; /*include*/
//                        echo "idPrioritas :" . $idPrioritas . "<br>"; /*exclude*/
//                        echo "kodeDivisi: " . $kode_divisi_pilihan . "<br>"; /*exclude*/
//                        echo "kodeSatker: " . $kode_satker_pilihan . "<br>"; /*exclude*/
//                        die;
                        $dtBerita = DB::select(DB::raw("select * from berita a inner join tbl_prioritas b 
                              on a.prioritas_id=b.id_prioritas where tgl_input between '$dari_rekap' and '$sampai_rekap'                               
                              order by b.skala_prioritas asc"));
                        //dd($dtBerita);
                    } else if ($idPrioritas != "all_priors") {
//                        echo $dari_rekap . "<br>"; /*include*/
...
//                        echo "idPrioritas :" . $idPrioritas . "<br>"; /*include*/
//                        echo "kodeDivisi: " . $kode_divisi_pilihan . "<br>"; /*exclude*/
//                        echo "kodeSatkerr: " . $kode_satker_pilihan . "<br>"; /*exclude*/
//                        die;
                        $dtBerita = DB::select(DB::raw("select * from berita a inner join tbl_prioritas b 
                              on a.prioritas_id=b.id_prioritas where prioritas_id='$idPrioritas' 
                              and tgl_input between '$dari_rekap' and '$sampai_rekap'                               
                              order by b.skala_prioritas asc"));
                        //dd($dtBerita);
                    }
                    $date = date('Y-m-d');
...
//                                order by b.skala_prioritas asc"));
                ...
                /*belum ada potensi masuk ke kondisi ini karena sudah di validasi harus terisi pada blade pilih_konfigurasi_kanwil.blade.php*/
                //echo "kode divisi tidak ada dipilih<br>";
//                $dtBerita = DB::table('berita')
//                    ->where('tgl_input', date('Y-m-d'))
//                    ->orderBy("prioritas_id", "asc")
//                    ->get();
...
//            $dtBerita = DB::table('berita')->where('tgl_input', date('Y-m-d'))->get();
            ...
            if (sizeof($dtBerita) != 0) {
                ...
                //todo lanjutkan disini triggerjo
                foreach ($dtBerita as $index => $news) {
                    ...
                    if ($getSatkerRole == 'humas_kanwil' || $getSatkerRole == 'humas_satker') {
                        //echo "woit"; die;
                        //$no += 1;
                        $no = $index + 1;
                        $text = $section->addText($no . ". " . $news->nama_berita, $firstStyle);
                ...
                $text = $section->addText("Dari : " . $dtKonfig->dari_pengirim, $secondtStyle);
                $text = $section->addText("");
                $text = $section->addText('Tembusan Ythssssssssssss:', $firstStyle);
                $jml_tembusan_yth = json_decode($dtKonfig->jumlah_tembusan_yth);
                foreach ($jml_tembusan_yth as $key => $dt) {
                    $nomer = $key + 1;
                    $text = $section->addText("$nomer ." . $dt, $secondtStyle);
...
//            $url_media_nasional_tosave = [];
    ...
    public function laporanberitapermedia()
    {
        ...
        //echo "laporan berita"; die;
        $judul_web = "Laporan Berita Per Media";
        $namabulan = [
            "",
            "Januari",
            "Februari",
    ...
    public function gantipassword($kode_satker, Request $request)
    {
        //return 'prety';
        $kd_satker = $kode_satker;
        //dd($request->kode_satker_pilihan);
    ...
    public function cetakrekapmediaexternal(Request $request)
    {
        ...
        $kode_divisi_pilihan = $request->kode_divisi_pilihan;
        //dd($request->kode_divisi_pilihan);
        ...
        if ($request->kode_divisi_pilihan == "all_berita") {
            ...
            //echo $request->kode_divisi_pilihan;
            $getberita = DB::table("berita")
                ->where('kode_satker', $kode_satker)
                ->where('kode_divisi', '=', 'berita_umum')
                ->whereBetween('tgl_input', [$dari, $sampai])
                ->get();
        } else {
            $getberita = DB::table("berita")
                ->where('kode_satker', $kode_satker)
                ->where('kode_divisi', '=', $request->kode_divisi_pilihan)
                ->whereBetween('tgl_input', [$dari, $sampai])
                ->get();
        }

        //echo "<pre>"; print_r($getberita);
        //die;
//        $getberita = DB::table("berita")
//            ->where('kode_satker', $kode_satker)
//            ->whereBetween('tgl_input', [$dari, $sampai])
        ...
        if ($request->jenis_media == 'media_lokal') {
            ...
            if (isset($_POST['exportexcelrekapexternal'])) {
                ...
                return view('laporan.cetakrekapexternal_excel_medialokal', compact('satker_name', 'tgl_dari', 'namabulan', 'bulan_dari',
                    'tahun_dari', 'tgl_sampai', 'bulan_sampai', 'tahun_sampai', 'getberita',
                    'mediapartner', 'kode_divisi_pilihan'));
            }
...
//        }
        ...
        if ($request->jenis_media == "sosial_media") {
            ...
            if (isset($_POST['exportexcelberita'])) {
                //dd($request);
                //echo "exportexcelberita media lokal pressed";die;
                $time = date('d-M-Y H:i:s');
                header("Content-type: application/vnd-ms-excel");
                header("Content-Disposition: attachment; filename=RekapitulasiLinkBeritaMediaLokal_$satker_name$time.xls");
    ...
    public function cetaklaporanberitapermedia(Request $request)
    {
        ...
        $kode_divisi_pilihan = $request->kode_divisi_pilihan ?? 'all_berita';

        // Pisahkan tanggal
        list($tahun_dari, $bulan_dari, $tgl_dari) = explode('-', $dari);
        list($tahun_sampai, $bulan_sampai, $tgl_sampai) = explode('-', $sampai);

        // --- Ambil data berita sesuai filter ---
        $getberita = DB::table('berita')
            ->where('kode_satker', $kode_satker)
            ->whereBetween('tgl_input', [$dari, $sampai])
            ->get();
        ...
        if ($jenis_media === 'sosial_media') {
            ...
            $countValid = function ($col) use ($kode_satker, $dari, $sampai) {
                return DB::table('berita')
                    ->where('kode_satker', $kode_satker)
                    ->whereBetween('tgl_input', [$dari, $sampai])
                    ->whereRaw("LENGTH($col) > 10")
                    ->count();
            };

            $total_website = 0;
            $total_facebook = $countValid('facebook');
            $total_instagram = $countValid('instagram');
            $total_twitter = $countValid('twitter');
            $total_tiktok = $countValid('tiktok');
            $total_sippn = $countValid('sippn');
            $total_youtube = $countValid('youtube');

            if (isset($_POST['exportexcelberita_permedia'])) {
                $time = date('d-M-Y_H-i-s');
                $filename = "RekapitulasiLinkSosialMedia_{$satker_name}_{$time}.xls";

                return response()
                    ->view('laporan.cetakrekapexternal_excel_sosmed_permedia', compact(
                        'filteredBerita',
                        'namabulan', 'tgl_dari', 'bulan_dari', 'tahun_dari',
                        'tgl_sampai', 'bulan_sampai', 'tahun_sampai',
                        'satker_name',
                        'total_website', 'total_facebook', 'total_instagram',
                        'total_twitter', 'total_tiktok', 'total_sippn', 'total_youtube'
                    ))
            ...
            return view('laporan.cetaklaporanberitapermedia', compact(
                'filteredBerita',
                'namabulan', 'tgl_dari', 'bulan_dari', 'tahun_dari',
                'tgl_sampai', 'bulan_sampai', 'tahun_sampai',
                'satker_name',
        ...
        foreach ($getberita as $berita) {
            ...
            foreach ($mediaArray as $entry) {
                ...
                // Filter media
                if (!empty($kode_media) && $kode_media !== 'semua' && $kode !== $kode_media) {
                    continue;
                }
        ...
        // 🔹 Ambil daftar media partner (diperbaiki untuk media nasional)
        if (!empty($kode_media) && $kode_media !== 'semua') {
            $mediapartner = DB::table('mediapartner')->where('kode_media', $kode_media)->get();
        } else {
            // Ambil semua media sesuai jenis media & satker
            $mediapartner = DB::table('mediapartner')
                ->where('jenis_media', $jenis_media)
                ->where(function ($q) use ($kode_satker) {
                    $q->where('kode_satker_penjalin', $kode_satker)
        ...
        if ($jenis_media === 'media_lokal') {
            if (isset($_POST['exportexcelberita_permedia'])) {
                $time = date('d-M-Y_H-i-s');
                $filename = "Rekap_Berita_Media_Lokal_{$satker_name}_{$time}.xls";

                header("Content-Type: application/vnd.ms-excel");
                header("Content-Disposition: attachment; filename=\"$filename\"");

                return view('laporan.cetakrekapexternal_excel_medialokal_permedia', compact(
                    'filteredBerita',
                    'namabulan',
                    'tgl_dari', 'bulan_dari', 'tahun_dari',
                    'tgl_sampai', 'bulan_sampai', 'tahun_sampai',
        ...
        elseif ($jenis_media === 'media_nasional') {
            ...
            // Hitung jumlah berita per media
            $countPerMedia = [];
            ...
            if (isset($_POST['exportexcelberita_permedia'])) {
                $time = date('d-M-Y_H-i-s');
                $filename = "Rekap_Berita_Media_Nasional_{$satker_name}_{$time}.xls";
                ...
                return view('laporan.cetakrekapexternal_excel_medianasional_permedia', compact(
                    ...
                    'tgl_sampai', 'bulan_sampai', 'tahun_sampai',
                    'satker_name', 'mediapartner', 'kode_divisi_pilihan', 'kode_media',
                    'countPerMedia', 'totalBerita'
                ));
            }

            return view('laporan.cetakrekapexternal_medianasional_perberita', compact(
                'filteredBerita',
                'namabulan',
                'tgl_dari', 'bulan_dari', 'tahun_dari',
                'tgl_sampai', 'bulan_sampai', 'tahun_sampai',
    ...
    public function cetaklaporanberitapermedia_olddua(Request $request)
    {
        ...
        // --- Olah data berita jadi array terstruktur untuk laporan ---
        $filteredBerita = [];
        foreach ($getberita as $berita) {
            $mediaArray = [];
            ...
            foreach ($mediaArray as $entry) {
                ...
                // Jika user pilih media tertentu, skip lainnya
                if (!empty($kode_media) && $kode_media !== 'semua' && $kode !== $kode_media) {
                    continue;
                }
        ...
        // --- Ambil data mediapartner sesuai filter ---
        if (!empty($kode_media) && $kode_media !== 'semua') {
            $mediapartner = DB::table('mediapartner')
                ->where('kode_media', $kode_media)
                ->get();
        } else {
            $mediapartner = DB::table('mediapartner')
                ->where('kode_satker_penjalin', $kode_satker)
        ...
        // --- Pilih tampilan berdasarkan jenis media ---
        if ($jenis_media === 'media_lokal') {
            return view('laporan.cetakrekapexternal_medialokal_perberita', compact(
                'filteredBerita',
                'namabulan',
                'tgl_dari', 'bulan_dari', 'tahun_dari',
                'tgl_sampai', 'bulan_sampai', 'tahun_sampai',
...
where kode_satker='$kode_satker' and length(youtube)>10 AND tgl_input between '$dari' and '$sampai'"));
        ...
        if ($request->jenis_media == 'media_lokal') {
            ...
            if (isset($_POST['exportexcelrekapexternal'])) {
                ...
                header("Content-type: application/vnd-ms-excel");
                header("Content-Disposition: attachment; filename=RekapitulasiLinkBeritaMediaLokal_$satker_name$time.xls");
                return view('laporan.cetakrekapexternal_excel_medialokal_perberita', compact('satker_name', 'tgl_dari', 'namabulan', 'bulan_dari',
                    'tahun_dari', 'tgl_sampai', 'bulan_sampai', 'tahun_sampai', 'getberita',
                    'mediapartner', 'kode_divisi_pilihan'));
            }

            //echo "<pre>"; print_r($getberita); die;
            return view('laporan.cetakrekapexternal_medialokal_perberita', compact('tgl_dari', 'namabulan', 'bulan_dari',
                'tahun_dari', 'tgl_sampai', 'bulan_sampai', 'tahun_sampai', 'getberita',
                'mediapartner', 'kode_divisi_pilihan'));
        } else if ($request->jenis_media == 'media_nasional') {
            $mediapartner = DB::table('mediapartner')->where('kode_satker_penjalin', $kode_satker)
            ...
            if (isset($_POST['exportexcelrekapexternal'])) {
                ...
                header("Content-type: application/vnd-ms-excel");
                header("Content-Disposition: attachment; filename=RekapitulasiLinkBeritaMediaNasional_$satker_name$time.xls");
                return view('laporan.cetakrekapexternal_excel_medianasional_perberita', compact('satker_name', 'tgl_dari', 'namabulan', 'bulan_dari',
                    'tahun_dari', 'tgl_sampai', 'bulan_sampai', 'tahun_sampai', 'getberita',
                    'mediapartner', 'kode_divisi_pilihan'));
            }
            return view('laporan.cetakrekapexternal_medianasional_perberita', compact('tgl_dari', 'namabulan', 'bulan_dari',
                'tahun_dari', 'tgl_sampai', 'bulan_sampai', 'tahun_sampai', 'getberita',
                'mediapartner', 'kode_divisi_pilihan'));
        }
        //die;
...
//        }
    ...
    public function storekonfig_rekap(Request $request)
    {
        ...
        try {
            ...
            $simpan = DB::table('konfigurasi_berita')->insert($data);
            if ($simpan) {
                return redirect()->route('konfig.konfigrekapberita')->with(['success' => 'Data Berhasil Disimpan']);
                //return Redirect::back()->with(['success' => 'Data Berhasil Disimpan']);
            } else {
                return redirect()->route('konfig.konfigrekapberita')->with(['warning' => 'Data Gagal Disimpan']);
                //return Redirect::back()->with(['warning' => 'Data Gagal Disimpan']);
            }
        } catch (\Exception $e) {
            return redirect()->route('konfig.konfigrekapberita')->with(['warning' => 'Data Gagal Disimpan, Galat:' . $e]);
            //return Redirect::back()->with(['warning' => 'Data Gagal Disimpan']);
        }
    }
    ...
    public function updatekonfig_rekap($id_konfig, Request $request)
    {
        ...
        try {
            ...
            $update = DB::table('konfigurasi_berita')->where('id_konfig', $id_konfig)->update($data_konfig_update);
            if ($update) {
                //herebro konfig.konfigrekapberita
                return redirect()->route('konfig.konfigrekapberita')->with(['success' => 'Data Konfigurasi Berhasil Diupdate']);
                //return Redirect::back()->with(['success' => 'Data Konfigurasi Berhasil Diupdate']);
            } else {
                return redirect()->route('konfig.konfigrekapberita')->with(['warning' => 'Data Konfigurasi Gagal Diupdate']);
